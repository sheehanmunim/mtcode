...
describe("bounded environment queries", () => {
  it.effect("fails when a connected query outlives its timeout", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const hangingSession = {
          client: { [WS_METHODS.serverGetUsageSummary]: () => Effect.never },
        } as unknown as RpcSession.RpcSession;
        const supervisor = EnvironmentSupervisor.EnvironmentSupervisor.of({
          target: TARGET,
          state: yield* SubscriptionRef.make(connectionState),
          session: yield* SubscriptionRef.make(Option.some(hangingSession)),
