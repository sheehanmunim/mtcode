...
// Lives in its own leaf module: `pullRequest.ts` needs it, and this module imports
// `pullRequest.ts` back for the PR snapshot schemas. Re-exported here so every existing
// `@t3tools/contracts` import site keeps reading it from `orchestration.ts`.
...
/**
 * Any non-image attachment (PDF, ZIP, source file). The mime type is
 * unconstrained on purpose: the provider adapters decide what they can accept,
 * and the size ceiling is the one `ServerEnvironment` advertises to clients as
...
export const OrchestrationThread = Schema.Struct({
  ...
  // Fractional index for user-arranged active (unsettled) order. Keyless
...
const ThreadActiveReorderCommand = Schema.Struct({
  ...
  commandId: CommandId,
  threadId: ThreadId,
  // Fractional index key for the active (unsettled) shelf: same one-write
  // scheme as ThreadPinReorderCommand. Clients compute a key that sorts
  // between the dropped position's neighbors.
  orderKey: TrimmedNonEmptyString,
});
