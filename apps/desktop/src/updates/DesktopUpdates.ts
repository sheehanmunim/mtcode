          ...
          admission = yield* stateMutex.withPermits(1)(
            Effect.gen(function* () {
              ...
              if (Option.isSome(activeAction)) {
                // Personal fork: also wait out a download. update-downloaded fires
                // from inside downloadUpdate, so the auto-install it triggers would
                // otherwise be refused for the reservation the download still holds.
                return (activeAction.value === "check" || activeAction.value === "download") &&
    ...
    yield* decodeUpdateInfo(raw).pipe(
      Effect.flatMap(
        Effect.fn("desktop.updates.applyUpdateDownloaded")(function* (info) {
          ...
          // Pin the install to the version that just finished downloading so a
          // superseded download is not installed (upstream's expectedVersion guard).
          const installResult = yield* installDownloadedUpdate(info.version);
