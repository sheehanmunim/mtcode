...
function buildRevertTurnCountByUserMessageId(input: {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    for (let nextIndex = index + 1; nextIndex < input.timelineEntries.length; nextIndex += 1) {
      ...
      // A checkpoint that is still resolving cannot be reverted to, so the
      // affordance stays hidden until it is ready.
      if (summary.status !== "ready" || summary.checkpointRef === null) {
        continue;
      }
      const turnCount =
