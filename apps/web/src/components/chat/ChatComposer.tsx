...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "slash-command") {
      ...
      const providerSlashCommandItems = getProviderSlashCommandsForSlashMenu(
        ...
        // The composer owns /goal; a provider command of the same name must
        // not shadow the built-in Goal entries.
    ...
    if (composerTrigger.kind === "pull-request") {
      // `#` serves two references: MT Code's cross-thread chips (fork) and
      // upstream's pull-request chips. Both lists share the one trigger, so
      // neither feature disappears when the other has no matches.
      const threadCandidates = threadShells.filter(
        (thread) => thread.environmentId === environmentId && thread.id !== routeThreadRef.threadId,
      );
      const threadItems = searchThreadReferences(threadCandidates, composerTrigger.query).map(
        (thread) => ({
      ...
      // A purely numeric ref (`#123`) means a pull request, so rank those
      // first; anything else is a thread query. Both lists stay visible either
      // way — ordering decides what Enter picks, not which feature survives.
      const pullRequestFirst = /^\p{N}+$/u.test(composerTrigger.query);
      const pullRequestItems = matches.map((pullRequest) => ({
      ...
      return pullRequestFirst
        ? [...pullRequestItems, ...threadItems]
        : [...threadItems, ...pullRequestItems];
    }
    return [];
  }, [
  ...
  const composerMenuEmptyState = useMemo(() => {
    ...
    if (composerTriggerKind === "pull-request") {
      ...
      return composerTrigger?.query
        ? `No thread or pull request matches ${composerTrigger.query}.`
        : "No threads or pull requests found.";
    }
  ...
  // Returns whether the prompt was stashed: the Objective badge's edit path only
  // replaces the draft once the typed prompt is safely parked.
      ...
      <div className="relative">
        ...
        >
          ...
          >
            ...
            {/* Bottom toolbar */}
            {(isComposerCollapsedMobile && voiceTranscription.status === "idle") ||
