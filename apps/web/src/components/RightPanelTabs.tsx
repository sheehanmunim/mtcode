              ...
              <Menu open={addSurfaceMenuOpen} onOpenChange={setAddSurfaceMenuOpen}>
                ...
                >
                  {addSurfaceActions.map((action) => {
                    const Icon = action.icon;
                    // Browser collapses into one row: clicking the row still opens
                    // the default one, so the common case stays a single click;
                    // hover or arrow reveals the rest. The choice lives at open
                    // time because a tab's profile is fixed then - Electron only
                    // honours a partition before attach.
                    if (
