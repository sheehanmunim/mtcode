...
const WINDOW: UsageSummaryInput = {
  ...
  // Without this the fork's hosted-client compat projection drops every
  // non-hosted provider (grok/cursor/opencode) from buckets and sources.
  clientContractVersion: USAGE_CONTRACT_VERSION,
};

const setup = Effect.gen(function* () {
  // Canonicalise: on macOS `tmpdir()` is `/var/...`, a symlink to
  ...
  ServerConfig.layerTest(process.cwd(), { prefix: input.prefix }).pipe(
    ...
    Layer.provideMerge(
      Layer.succeed(HostProcessEnvironment, {
        ...
        // Point OpenCode discovery inside the fixture. Without this the scan
        // falls back to the real `~/.local/share/opencode`, so `sources`
