...
function withRateLimitBackoff(
  ...
  const wrapped = {
    ...
    ...(api.listReleases === undefined
      ? {}
      : { listReleases: wrap("listReleases", api.listReleases) }),
...
export const make = Effect.gen(function* () {
    ...
    listWorkspaceProjects({ projectId: ref.projectId }).pipe(
      Effect.flatMap(({ supported }): Effect.Effect<SupportedProject, PullRequestError> => {
        ...
        if (host === undefined) {
          ...
          // own remote rather than being handed to a provider verbatim. A row the listing read
          // from the project's upstream is opened through the same project, so the check has to
          // admit that repository too — still a repository this service chose, never one the
          // client named. A read the listing could not have produced still fails.
          const wanted = repository.toLowerCase();
          return resolveUpstream(own).pipe(
            Effect.flatMap((upstream) =>
              upstream !== null && upstream.toLowerCase() === wanted
                ? Effect.succeed({ ...own, repository: upstream, isUpstream: true })
                : Effect.fail(
                    new PullRequestOperationError({
  ...
  const refEpoch = (ref: PullRequestRef) => Math.max(turnRefreshEpoch, refEpochs.get(ref));
        ...
        Effect.andThen(
          Effect.sync(() => {
            if (input.resource === "diff") {
              bumpAggregateDiffEpoch(reference);
              return;
            }
