...
const CURSOR_ACP_MODEL_DISCOVERY_FAILED_MESSAGE = [
  ...
  `Cursor CLI setup may be incomplete; install or enable the Cursor CLI, restart ${resolveAppDisplayName()}, and try again.`,
...
function buildCursorCliCommandMissingMessage(binaryPath: string): string {
  return [
    ...
    `Install or enable the Cursor CLI, make sure \`${binaryPath}\` is on PATH, then restart ${resolveAppDisplayName()}.`,
    `See ${CURSOR_CLI_INSTALLATION_DOCS_URL}.`,
  ].join(" ");
}
...
> {
  ...
  if (parsed.auth.status !== "unauthenticated") {
    ...
    const discoveryExit = yield* Effect.exit(
      ...
      ).pipe(Effect.timeoutOption(acpDiscoveryTimeoutMs)),
    );
