...
export function mapCodexStatus(
  ...
  // The usage probe degrades to a `failure` message, and its success shape is
  // the app-server snapshot rather than the old `rateLimits.rateLimits` blob.
  const probe = snapshot.rateLimits;
  const rateLimits = probe !== undefined && !("failure" in probe) ? probe.snapshot : undefined;
  ...
  return {
    ...
    ...(rateLimits
      ? {
          rateLimits: {
            ...(rateLimits.planType !== undefined ? { limitName: rateLimits.planType } : {}),
