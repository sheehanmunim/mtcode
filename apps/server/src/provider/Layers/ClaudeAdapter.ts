...
import { makeClaudeUserDefinesDesktopMcp } from "../../desktopControl/desktopMcpUserConfig.ts";
...
interface ClaudeSessionContext {
  ...
  queryReady: Deferred.Deferred<void>;
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const userDefinesDesktopMcp = yield* makeClaudeUserDefinesDesktopMcp();
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    switch (message.subtype) {
      ...
      case "compact_boundary": {
        ...
        const compactedUsage = yield* resolveCompactBoundaryTokenUsage(
          context,
  ...
  const emitAccountUsageSnapshot = Effect.fn("emitAccountUsageSnapshot")(function* (
    ...
    const stamp = yield* makeEventStamp();
    // Only the limit windows travel: the full response also carries session
    ...
    // The probe is the only source that names the scoped buckets, so hand the
    // names to the streamed-event mapper before the next `rate_limit_event`.
  ...
  const replaceQueryAfterRollback = Effect.fn("replaceQueryAfterRollback")(function* (
    ...
    context.queryReady = yield* Deferred.make<void>();
    yield* updateResumeCursor(context);
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      // restarting the app. User config wins: when the user's own Claude config
      // already defines a `mt-desktop` server, skip injection so the
      // SDK-supplied entry cannot shadow theirs.
      const resolvedDesktopMcp = yield* resolveDesktopMcp();
      const desktopMcp =
        resolvedDesktopMcp !== undefined &&
        !(yield* userDefinesDesktopMcp({
          ...(input.cwd ? { cwd: input.cwd } : {}),
  ...
  const sendTurn: ClaudeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
    ...
    if (needsModelUpdate || needsPermissionUpdate) {
      ...
      if (queryReady) {
        if (needsModelUpdate && nextApiModelId !== undefined) {
          ...
          if (applied && nextApiModelId !== undefined) {
            context.queryOptions = {
              ...context.queryOptions,
              model: nextApiModelId,
            };
          }
        }
        if (needsPermissionUpdate && desiredPermissionMode !== undefined) {
          ...
          if (applied) {
            context.queryOptions = {
              ...
              ...(desiredPermissionMode === "bypassPermissions"
                ? { allowDangerouslySkipPermissions: true }
                : {}),
            };
          }
    ...
    if (steeringTurnState === null) {
      ...
      context.announcedUsageLimits = undefined;
      context.session = {
        ...context.session,
        status: "running",
        activeTurnId: turnId,
