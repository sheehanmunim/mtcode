    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        if (desktopMcp) {
          ...
          appServerArgs.push(
            "-c",
            `mcp_servers.${DESKTOP_MCP_SERVER_NAME}.command=${quoteToml(desktopMcp.path)}`,
          );
        ...
        const eventFiber = yield* Stream.runForEach(runtime.events, (event) =>
          Effect.gen(function* () {
            ...
            const runtimeEvents = mapToRuntimeEvents(event, event.threadId, collabUsageByAgent).map(
              (runtimeEvent) => {
