...
function makeHarness(config?: {
  ...
  // Every `createQuery` rotates in a fresh query that adopts the previous
  // query's stubs (fork), and `queries` records each instance the adapter was
  // handed (upstream) so tests can reach the one created after a recovery.
  const queries: FakeClaudeQuery[] = [new FakeClaudeQuery()];
  let createInput:
    | {
...
describe("ClaudeAdapterLive", () => {
      ...
      const permissionResult = yield* Effect.promise(() =>
        canUseTool(
          ...
          { command: "pwd" },
          {
            ...
            requestId: "request-tool-use-bypass",
  ...
  it.effect("does not auto-allow tools when full access is policy-downgraded to supervised", () => {
    ...
    return Effect.gen(function* () {
      ...
      const permissionPromise = canUseTool(
        ...
        {
          signal: new AbortController().signal,
          requestId: "request-tool-use-supervised",
  ...
  it.effect("puts the command text last so attachments do not suppress expansion", () => {
    ...
    return Effect.gen(function* () {
      ...
      // PDF attachments ride along as document blocks ahead of the trailing
      // command text, matching buildClaudePdfContentBlock in the adapter.
  ...
  it.effect("keeps post-compact usage when turn completion follows a compact boundary", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      // The result's cumulative totals describe the pre-compact context; the
      // meter keeps the post-compact figure and only the processed total grows.
      assert.equal(usageEvents[1]?.type, "thread.token-usage.updated");
      if (usageEvents[1]?.type === "thread.token-usage.updated") {
        assert.equal(usageEvents[1].payload.usage.usedTokens, 18_000);
        assert.equal(usageEvents[1].payload.usage.totalProcessedTokens, 24_542);
        assert.equal(usageEvents[1].payload.usage.maxTokens, 200000);
      }
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
  ...
  it.effect("replaceQueryAfterRollback keeps mid-session model and permission mode", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(restarted?.options.model, `${SYNTHETIC_CLAUDE_CAPABLE_MODEL}[expanded]`);
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        if (turnStarted._tag === "Some" && turnStarted.value.type === "turn.started") {
          assert.deepEqual(turnStarted.value.payload, { model: SYNTHETIC_CLAUDE_CAPABLE_MODEL });
        }
        const sessions = yield* adapter.listSessions();
        assert.equal(sessions[0]?.model, SYNTHETIC_CLAUDE_CAPABLE_MODEL);
        assert.deepEqual(harness.query.setModelCalls, [
          `${SYNTHETIC_CLAUDE_CAPABLE_MODEL}[expanded]`,
        ]);
      }).pipe(
  ...
  it.effect("does not emit proposed plan while user input is pending", () => {
    ...
    return Effect.gen(function* () {
      ...
      const permissionPromise = canUseTool(
        ...
        {
          signal: new AbortController().signal,
          requestId: "request-tool-ask-pending-plan",
