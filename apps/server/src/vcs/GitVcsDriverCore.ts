...
// Scratch indexes live outside .git/index, but git still honours the repository's
// core.splitIndex setting when it writes one. That would drop a fresh
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const readUntrackedNumstat = Effect.fn("readUntrackedNumstat")(function* (
    ...
    const result = yield* executeGitWithStableDiagnostics(
      operation,
      cwd,
      [...TEMP_INDEX_CONFIG_ARGS, "diff", "--numstat", "--"],
      {
        env,
        maxOutputBytes: STATUS_UNTRACKED_NUMSTAT_MAX_PATH_BYTES,
        appendTruncationMarker: true,
  ...
  const pushCurrentBranch: GitVcsDriver.GitVcsDriver["Service"]["pushCurrentBranch"] = Effect.fn(
    ...
    if (currentUpstream) {
      ...
      if (!isAliasOfUpstreamHead) {
        ...
        const ownBranchSshAuthentication =
          options?.sshAuthentication ??
          (yield* resolveRemoteSshAuthentication(cwd, remoteName, "push"));
        yield* runGit(
          "GitVcsDriver.pushCurrentBranch.pushOwnBranch",
          cwd,
          ["push", "-u", remoteName, `HEAD:refs/heads/${publishBranch}`],
          { timeoutMs: null, sshAuthentication: ownBranchSshAuthentication },
        );
        return {
          status: "pushed" as const,
          branch,
