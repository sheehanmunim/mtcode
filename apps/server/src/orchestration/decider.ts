...
// recent 500 plus pending async questions. That bound is safe here: an OPEN
// approval/user-input request blocks its turn, so the thread cannot accumulate
// hundreds of later activities while one is outstanding — a request that has
// scrolled out of the window is one whose turn kept running, i.e. it was
// resolved or went stale. Async questions remain actionable while the agent
// works, so they must not expire with the activity window. (The projection
// pipeline's pendingApprovalCount reads the same capped stream and stays
 ...
 * Durable after-current messages carry an explicit delivery state and remain
 * work until dispatched or cancelled, regardless of the adoption grace window.
 */
...
export function isThreadIdleForGoal(
  ...
  return !hasQueuedTurnStartForThread(thread, occurredAt);
}
