...
/**
 * The fork's Goal loop asks the same question as a plain boolean. The activity
 * window bound is safe here: an OPEN approval/user-input request blocks its
 * turn, so the thread cannot accumulate hundreds of later activities while one
 * is outstanding — a request that has scrolled out of the window is one whose
 * turn kept running, i.e. it was resolved or went stale.
 */
export function hasOpenBlockingRequest(thread: Pick<OrchestrationThread, "activities">): boolean {
