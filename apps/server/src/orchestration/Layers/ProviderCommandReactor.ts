...
const make = Effect.gen(function* () {
  ...
  const cancelTurnsAfterCompaction = Effect.fn("cancelTurnsAfterCompaction")(function* (
    ...
    for (const event of queued) {
      yield* appendProviderFailureActivity({
        ...
        // A message-less Continuation carries no messageId, and
        // `exactOptionalPropertyTypes` refuses an explicit `undefined`.
        ...(event.payload.messageId === undefined ? {} : { requestId: event.payload.messageId }),
  ...
  const resumeTurnsAfterCompaction = Effect.fn("resumeTurnsAfterCompaction")(function* (
    ...
    while (queued.length > 0 && turnsAfterCompaction.get(threadId) === queued) {
      ...
      // A message-less Continuation (the fork's Goals path) has no user message
      // to restore, so there is no turn-start row to replay it from. Drop it
      // from the queue rather than looking up `undefined`.
      const queuedMessageId = event.payload.messageId;
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    const authCommandHandled = yield* Effect.gen(function* () {
      // A message-less start (a queued continuation) carries no prompt text,
      // so there is no slash command for the provider to claim.
    ...
    // Upstream expands the composer's context references before the provider sees the text.
    // A message-less Continuation carries no composer context, so its T3-authored prompt
  ...
  const recoverAmbiguousQueuedTurnHandoff = Effect.fn("recoverAmbiguousQueuedTurnHandoff")(
    function* (row: ProjectionQueuedTurns.ProjectionQueuedTurn) {
      const thread = yield* resolveThreadDetail(row.threadId);
      if (thread === undefined) {
        return;
      }
