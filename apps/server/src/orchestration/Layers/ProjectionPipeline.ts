...
import {
  ...
  layer as ProjectionQueuedTurnRepositoryLive,
} from "../../persistence/ProjectionQueuedTurns.ts";
...
const makeOrchestrationProjectionPipeline = Effect.fn("makeOrchestrationProjectionPipeline")(
  function* () {
    ...
    const applyQueuedTurnsProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        case "thread.turn-queued":
          // Goal continuations queue without a user message, so there is no
          // message row to key the queue entry on.
          if (event.payload.messageId === undefined) return;
        ...
        case "thread.queued-turn-dispatched":
          yield* projectionQueuedTurnRepository.markHandoff({
            messageId: event.payload.messageId,
          });
          return;
    ...
    const applyThreadTurnsProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.turn-start-requested": {
          ...
          // A pending start can be message-less (a queued continuation), and
          // only a real user message can be the `/compact` this skips.
