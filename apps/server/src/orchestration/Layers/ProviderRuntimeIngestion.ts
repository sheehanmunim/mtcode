  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      if (
        ...
        // A Goal Continuation starts a turn with no user message, so there is
        // no pending message to inspect for a /compact command.
        const pendingMessageId = pendingTurnStart.value.messageId;
        const pendingMessage =
          pendingMessageId === null
            ? null
            : yield* getThreadMessageById(thread.id, pendingMessageId);
        if (
