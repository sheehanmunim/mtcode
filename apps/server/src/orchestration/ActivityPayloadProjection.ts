...
export function projectActivityPayload(
  ...
  return {
    ...
    payload: {
      ...
      // Read `detail` off `statusPayload` rather than `projectedPayload`:
      // upstream's favicon work made `projectPreviewToolMetadata` return a
      // `toolIcon` union, and spreading a union hides the property. The two are
      // the same value, since `statusPayload` is spread last.
      ...(imagePath && asTrimmedString(statusPayload.detail) === null ? { detail: imagePath } : {}),
      data: projectedData,
    },
  };
}
