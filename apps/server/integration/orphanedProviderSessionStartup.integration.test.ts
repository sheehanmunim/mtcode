...
import { SessionStartupReconcilerLive } from "../src/provider/Layers/SessionStartupReconciler.ts";
...
it.effect("queues persisted work for resume before opening the command gate after restart", () =>
  Effect.gen(function* () {
    ...
    const reconciliationLayer = SessionStartupReconcilerLive.pipe(
    ...
    const startupLayer = ServerRuntimeStartup.layer.pipe(
      Layer.provideMerge(secondRuntime),
      Layer.provideMerge(startupDependencies),
      Layer.provideMerge(reconciliationLayer),
    ...
    const result = yield* Effect.gen(function* () {
      ...
      const stoppedBinding = Option.getOrThrow(yield* directory.getBinding(stoppedBindingThreadId));
    ...
    assert.deepStrictEqual(result, {
      ...
      latestTurn: null,
      pendingTurnCount: 2,
      settleSucceeded: false,
      snoozeSucceeded: false,
